...
def test_normalize_url_collapses_differences_that_do_not_change_the_file(a, b):
    assert atlas_registry.normalize_url(a) == atlas_registry.normalize_url(b)
...
def test_normalize_url_keeps_differences_that_might_matter(a, b):
    assert atlas_registry.normalize_url(a) != atlas_registry.normalize_url(b)


def test_normalize_url_handles_empty_input():
    assert atlas_registry.normalize_url("") == ""
    assert atlas_registry.normalize_url(None) == ""
...
def test_normalize_ntd_id(raw, expected):
    assert atlas_registry.normalize_ntd_id(raw) == expected
