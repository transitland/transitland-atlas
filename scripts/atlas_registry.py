...
def onestop_id_name_advisories(osid: str) -> list[str]:
    ...
    Separate from `onestop_id_problems` and never fatal. The scheme allows
    alphanumerics from any script plus ASCII `~`, but the Japanese and GBFS
    feeds are generated by sync jobs that mint ids from agency names, so
    underscores and full-width punctuation arrive without a human in the loop.
    Failing the build on those would block an automated update rather than
    reach whoever could change the generator.
    """
    if not osid:
        return []
