...
# inconsistent, so both sides are normalized before joining.
NTD_WIDTH = 5
...
def normalize_url(url: str) -> str:
    ...
    release, normalizing this way moved 70 agencies out of the unmatched bucket.
...
def normalize_ntd_id(value: str) -> str:
...
def _url_index(db, historic: bool) -> dict[str, set[str]]:
    """Normalized URL -> feeds, over either the active or the superseded fields.
    ...
    for row in db.execute(
        ...
        if row["kind"] == "array":
            ...
            values = [row["value"]]
        for url in values:
            out.setdefault(normalize_url(url), set()).add(row["id"])
    return out


def active_urls(db) -> dict[str, set[str]]:
    """Normalized active URL -> feeds using it. Superseded URLs are excluded."""
    return _url_index(db, historic=False)


def historic_urls(db) -> dict[str, set[str]]:
    """Normalized superseded URL -> feeds that record it. A source still
...
def malformed_ntd_ids(db) -> list[tuple[str, str]]:
    ...
    are silently corrected by `normalize_ntd_id`; they are still worth fixing at
